all: mobile/src/main/jniLibs/armeabi/libvpn.so

.PHONY: clean

clean:
	rm -rf mobile/src/main/jni/ShadowVPN/shadowvpn-android-*
	rm -rf mobile/libs
	rm -rf mobile/src/main/jniLibs

mobile/src/main/jniLibs/armeabi/libvpn.so: jni/vpn.c jni/Android.mk
	if [ a == a$(ANDROID_NDK_HOME) ]; then \
		echo ANDROID_NDK_HOME is not set ;\
		exit 1 ;\
	fi ;\
	pushd mobile/src/main/jni/ShadowVPN || exit 1 ;\
	if [ ! -f configure ]; then \
    	./autogen.sh || exit 1 ;\
    fi ;\
